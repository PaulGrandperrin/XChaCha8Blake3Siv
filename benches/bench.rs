...
fn bench(c: &mut Criterion) {
    ...
    let mut group_xchacha20blake3siv = c.benchmark_group("xchacha20blake3siv_encrypt");
    for size in [1, 32, 128, 4 * KB, 64 * KB].iter() {
        ...
        group_xchacha20blake3siv.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            b.iter(|| cipher_xchacha20blake3siv.encrypt_in_place_detached(nonce, associated_data, &mut buffer[0..size]));
        });
    }
    group_xchacha20blake3siv.finish();

    let mut group_xchacha20poly1305 = c.benchmark_group("xchacha20poly1305_encrypt");
    for size in [1, 32, 128, 4 * KB, 64 * KB].iter() {
        ...
        group_xchacha20poly1305.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            b.iter(|| cipher_xchacha20poly1305.encrypt_in_place_detached(nonce, associated_data, &mut buffer[0..size]));
        });
    }
